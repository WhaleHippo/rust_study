...
impl Guess {
    fn new(value: u32) -> Self {
        ...
        // C11-01의 포함 경계는 100이다. `<=`가 이를 허용하고 101부터 기존 패닉 의미를 유지한다.
        assert!(
            value <= 100,
...
mod tests {
    ...
    // [학습 실습: C11-01]
    // 목표: 포함 경계와 패닉으로 지키는 생성자 불변식을 구분한다.
    // 학습자 행동: `<= 100`을 `< 100`으로 잠시 바꾸고 아래 테스트만 실행한다.
    // RED: `cargo test -p chapter11-writing-automated-tests guess_new_when_value_is_upper_boundary`는 100에서 패닉하여 실패한다.
    // GREEN: `<= 100`으로 복원해 100을 허용하고 101의 패닉 의미를 유지한다.
    // 힌트: 상한 경계는 조건식의 등호 하나로 결정된다.
    #[test]
    fn guess_new_when_value_is_upper_boundary() {
        // 준비: 허용되는 가장 큰 Guess 값 100을 준비한다.
        // 실행: 포함 경계 값으로 Guess를 생성한다.
        let guess = Guess::new(100);
        // 검증: 생성자는 유효한 포함 경계를 보존한다.
