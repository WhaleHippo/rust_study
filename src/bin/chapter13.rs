...
fn main() {
    ...
    println!("captures: Fn={fn_value}, FnMut={fn_mut_values:?}, FnOnce={fn_once_label}");
    println!("adapters: {:?}", doubled_evens(&[1, 2, 3, 4]));
    println!("custom Iterator: {:?}", Counter::new(3).collect::<Vec<_>>());
}
