...
fn increment_shared(worker_count: usize) -> Result<usize, LessonError> {
    ...
    // 요청한 작업자 수만큼 반복해야 각 작업자가 카운터를 정확히 한 번 증가시킨다.
...
mod tests {
    ...
    // 학습자 행동: 작업자 범위를 로컬에서 `0..worker_count.saturating_sub(1)`로 바꾼 뒤, 요청한 수만큼 스레드를 만드는 범위를 직접 다시 구현한다.
    // RED: 워크스페이스 루트에서 `cargo test -p chapter16-fearless-concurrency one_worker_increments_shared_counter_once`를 실행하면 1 기대값 대신 0을 받아 실패한다.
    // GREEN: 정확히 `0..worker_count` 작업자를 만들도록 복원하면 카운터가 1이 된다.
    // 힌트: `worker_count`가 1일 때 두 범위가 각각 몇 번 반복되는지 비교한다.
    #[test]
    fn one_worker_increments_shared_counter_once() {
        // 준비: 동기화된 카운터를 공유하는 작업자 한 명을 준비한다.

        // 실행: 작업자가 한 번 증가시킨 뒤 join한다.
        let count = increment_shared(1).expect("the worker should finish");

        // 검증: 정확히 한 번의 갱신이 기록된다.
        assert_eq!(count, 1);
    }
}
