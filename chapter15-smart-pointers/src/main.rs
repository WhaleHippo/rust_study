...
impl List {
    fn sum(&self) -> i32 {
        ...
        match self {
            // 현재 값에 다음 목록의 합을 더해야 재귀 목록의 모든 노드가 누적된다.
...
mod tests {
    ...
    // [학습 실습: C15-01]
    // 목표: 재귀 목록의 세 노드를 모두 순회해 합 6을 만드는 계약을 고정한다.
    // 학습자 행동: `sum`의 재귀 팔을 로컬에서 현재 값만 반환하도록 바꾼 뒤, 다음 노드까지 더하는 재귀를 직접 다시 구현한다.
    // RED: 워크스페이스 루트에서 `cargo test -p chapter15-smart-pointers recursive_list_sums_three_nodes`를 실행하면 6 기대값 대신 1을 받아 실패한다.
    // GREEN: `value + next.sum()`을 복원하면 세 노드 합 테스트가 통과한다.
    // 힌트: `Cons`는 현재 `value`와 `Box`가 가리키는 나머지 목록을 함께 가진다.
    #[test]
    fn recursive_list_sums_three_nodes() {
        // 준비: Box 간접 참조로 연결한 세 노드 재귀 목록을 준비한다.
        let list = List::Cons(
            1,
            Box::new(List::Cons(2, Box::new(List::Cons(3, Box::new(List::Nil))))),
        );

        // 실행: 모든 노드를 재귀적으로 순회해 합을 구한다.
        let total = list.sum();

        // 검증: 세 값이 모두 합 6에 기여한다.
        assert_eq!(total, 6);
    }
