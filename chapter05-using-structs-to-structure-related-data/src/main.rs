...
impl Rectangle {
    ...
    // 높이 비교를 빼면 너비가 맞고 높이가 큰 후보를 잘못 포함하게 되므로 `&&`가 두 조건을 묶는다.
...
mod tests {
    ...
    // 학습자 행동: 로컬에서 높이 조건을 임시 제거한 뒤 두 차원 비교를 직접 다시 구현한다.
    // RED: `cargo test -p chapter05-using-structs-to-structure-related-data rejects_rectangle_when_one_dimension_is_too_large`에서 높이가 큰 후보가 잘못 허용되어 실패한다.
    // GREEN: 너비와 높이 비교를 `&&`로 묶으면 이 테스트와 장 전체 테스트가 통과한다.
    // 힌트: 포함되려면 `width`와 `height` 조건이 모두 참이어야 한다.
    #[test]
